          ...
          element?.Routes?.forEach((route:any)=>{
            // route?.Locations?.forEach((loc:any,i:any)=>{
              this.draggeditem.forEach((item:any)=>{
                // if(item?.LocationId == loc?.LocationId) route?.Locations?.splice(i,1);
               route.Locations =  route?.Locations?.filter((_:any,index:any)=>{return (_?.LocationId != item?.LocationId)})
              })              
            // })
          })
        });
